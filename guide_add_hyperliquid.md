# ⚡ Guide: Adding Hyperliquid (HL) Exchange Connector
...
exchanges/
└── hyperliquid/
    ├── api_client.py      # REST API implementation
    └── ws_client.py       # WebSocket streams (candles, trades)
```
...
|------------|--------|------------------------|-------|
| OHLC (live + historical) | WebSocket | `{"type": "candle", "coin": "BTC", "interval": "1m"}` | Provides live and backfilled candles |
| Open Interest | REST | `{ "type": "metaAndAssetCtxs" }` | Includes OI and market context |
...
Handles **open interest** and **funding rate** data.

```python
# exchanges/hyperliquid/api_client.py

import aiohttp
from core.schemas import OpenInterest, FundingRate
...
| OHLC Data | ✅ WS supported |

---

## 🧭 Summary

Hyperliquid is now ready to be added with minimal effort:
...
Once done, HL data will flow into your normalized backend just like Binance — ready for charts, metrics, and analytics.
